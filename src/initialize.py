...
def get_start_values(table_path, sd_card_name):
    '''
        Starting from the bottom up in the migration table, look for the most recent migration with the given SD card name
...
def edge_case_9999(start_dir):
    ...
    os.makedirs(new_dir)
    return new_dir, "IMG_0001.JPG"

def initialize_repo(sd_card_name, external_hd_name, table_path):
    # does_path_exist(f"/Volumes/{sd_card_name}")
    # does_path_exist(f"/Volumes/{external_hd_name}")
